/*  $Id: SwitchStmt.cpp,v 1.17 2022/01/31 23:35:47 sarrazip Exp $
...
SwitchStmt::emitCode(ASMText &out, bool lValue) const
{
    ...
    out.emitComment("Switch at " + expression->getLineNo()
                            + " uses " + (jumpMode ? "jump table" : "if-else sequence")
                            + ": IF_ELSE=" + dwordToString(uint32_t(ifElseCost))
                            + ", JUMP_TABLE=" + dwordToString(uint32_t(jumpTableCost)));
